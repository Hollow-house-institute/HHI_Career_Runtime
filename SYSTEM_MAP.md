# Hollow House Institute (HHI)
## Repository System Map

Founder: Amy Pierce Bui

---

# Overview

The HHI repository ecosystem is organized into layered governance infrastructure components.

Each repository serves a distinct operational role within the broader Behavioral AI Governance framework.

---

# Canonical Governance Structure

## 1. Hollow_House_Standards_Library
Role:
Canonical terminology and standards authority.

Purpose:
- governance definitions
- glossary authority
- governance architecture
- standards crosswalks
- terminology continuity

Repository:
https://github.com/Hollow-house-institute/Hollow_House_Standards_Library

---

## 2. HHI_GOV_01
Role:
Execution-Time Governance framework.

Purpose:
- runtime governance
- Decision Boundary enforcement
- Stop Authority structures
- governance telemetry
- runtime accountability models
- continuous assurance structures

Repository:
https://github.com/Hollow-house-institute/HHI_GOV_01

---

## 3. HHI_Runtime_Proof
Role:
Operational runtime evidence layer.

Purpose:
- telemetry proof
- runtime screenshots
- governance observability
- deployment continuity
- operational evidence preservation

Repository:
https://github.com/Hollow-house-institute/HHI_Runtime_Proof

---

## 4. HHI_Audits
Role:
Governance audit and interaction evidence repository.

Purpose:
- case studies
- governance interaction evidence
- co-creation lineage
- replay-capable audit continuity
- longitudinal accountability evidence

Repository:
https://github.com/Hollow-house-institute/HHI_Audits

---

## 5. HHI_Career_Runtime
Role:
Professional portfolio and executive translation layer.

Purpose:
- executive overview
- diagrams
- services
- public positioning
- operational portfolio proof
- repository navigation

Repository:
https://github.com/Hollow-house-institute/HHI_Career_Runtime

---

# Governance Architecture Flow

Standards Library
↓
HHI_GOV_01
↓
Runtime Proof
↓
Audits and Case Studies
↓
Executive Translation and Public Positioning

---

# Operational Governance Themes

- Behavioral Drift
- Execution-Time Governance
- Governance Telemetry
- Decision Boundaries
- Stop Authority
- Longitudinal Accountability
- Runtime Observability
- Human-in-the-Loop Oversight

---

# Public Authority References

GitHub Organization:
https://github.com/Hollow-house-institute

DOI:
https://doi.org/10.5281/zenodo.20044740

ORCID:
https://orcid.org/0009-0009-4806-1949

LinkedIn:
https://www.linkedin.com/in/hollow-house-institute-3ab5a2182

---

Amy Pierce Bui
Founder, Hollow House Institute
